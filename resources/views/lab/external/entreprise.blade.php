    ...
    <form method="POST" action="{{ route('lab.external.entreprise.store') }}"
        ...
        <div style="margin-bottom:20px;">
            <label for="raison_sociale" style="display:block;font-size:var(--text-sm);font-weight:500;color:var(--fg-primary);margin-bottom:6px;">
                Raison sociale <span style="color:var(--color-brique-600);">*</span>
            </label>
            <input type="text" id="raison_sociale" name="raison_sociale"
                   value="{{ old('raison_sociale') }}"
                   placeholder="Ma Société SAS"
                   style="width:100%;padding:10px 12px;border:1px solid {{ $errors->has('raison_sociale') ? 'var(--color-brique-600)' : 'var(--border-default)' }};border-radius:var(--radius-md);font-size:var(--text-sm);background:var(--bg-base);color:var(--fg-primary);box-sizing:border-box;">
        ...
        <div style="margin-bottom:20px;">
            <label for="nom_contact" style="display:block;font-size:var(--text-sm);font-weight:500;color:var(--fg-primary);margin-bottom:6px;">
                Nom du contact <span style="color:var(--color-brique-600);">*</span>
            </label>
        ...
        <div style="margin-bottom:20px;">
            <label style="display:block;font-size:var(--text-sm);font-weight:500;color:var(--fg-primary);margin-bottom:10px;">
                Type de besoin <span style="font-weight:400;color:var(--fg-tertiary);">(optionnel)</span>
            </label>
            <div style="display:flex;gap:16px;flex-wrap:wrap;">
                @foreach(['facilitation' => 'Facilitation', 'innovation' => 'Innovation', 'autre' => 'Autre'] as $value => $label)
                    <label style="display:flex;align-items:center;gap:8px;cursor:pointer;font-size:var(--text-sm);color:var(--fg-secondary);">
                        <input type="radio" name="besoin_type" value="{{ $value }}"
                               {{ old('besoin_type') === $value ? 'checked' : '' }}>
                        {{ $label }}
                    </label>
                @endforeach
            </div>
        ...
        <div style="margin-bottom:24px;">
            <label for="message" style="display:block;font-size:var(--text-sm);font-weight:500;color:var(--fg-primary);margin-bottom:6px;">
                Description du besoin <span style="color:var(--color-brique-600);">*</span>
            </label>
